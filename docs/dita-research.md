# DITA research for Verso — ideas and tooling

Research synthesis, August 2026. Two workstreams: DITA concepts mapped onto Verso, and the DITA tooling ecosystem surveyed for borrowable patterns.

**Framing.** DITA and Verso share a thesis: structure authored once should project into many outputs. DITA projects topics into HTML/PDF/etc.; Verso projects one tree into HTML + JSON-LD. The meta-lesson from the OASIS committee itself: when they built **LwDITA**, they kept *typed topics, maps, conref, and keys* — and dropped specialization machinery, grammar files, and pipelines. MDITA even reached for YAML front matter for metadata. Verso already *is* an LwDITA in spirit.

The through-line: DITA spent decades separating **content, composition, and delivery**; Verso unified **presentation and assertion**. Import the separation as *data in the tree*, never as a build pipeline.
...
| DITA concept | Verso analog today | Verdict |
|---|---|---|
...
The map owns the publication; fragments own content. Verso twist DITA never had: a map is *also* graph composition — one structure, three projections (HTML document, nav, site-level knowledge graph via `CollectionPage`/`hasPart`).
...
3. **"Schematron for Verso"** — rules-as-data over the *resolved* tree: selector + predicate + message/severity + phase (`author` phase live in dev server, `publish` gating CLI). Lint results as a third live WebSocket panel. Grammar (strict mode) plateaus; rules are where teams actually accumulate governance.
...
Notable: oXygen 28 added an **AI-friendly output format** — "agents as a publishing target" is literally Verso's JSON-LD home turf. LwDITA's adapter pattern (thin parser lifting another syntax into the same tree) is right, but deferred until a second authoring syntax has real demand.

## Anti-goals

Anything requiring a build stage, grammar compilation, or processing-order law is disqualifying — Verso's edge is single-pass rendering that re-evaluates live.
...
7. How loud should typing be? `requires:` as strict error vs warning-only (`VERSO_STRICT=2`?).
